...
if ($u->getMsg() == '') {
    $resultado = $u->readChecked($id);
    if (isset($_POST['submit']) && !empty($_POST['nome']) && !empty($_POST['id'])) {
        if (!empty(($_POST['ckTag']))) {
            $nome = addslashes($_POST['nome']);
            $id = $_POST['id'];
            $tags = $_POST['ckTag'];
            if ($u->editProduto($nome, $id, $tags)) {
            ...
            $u->msg("É necessário selecionar pelo menos uma tag para editar o produto.", 'danger');
        }
    }
}
?>
...
<body>
    ...
    <main class="container">
        <form action="edicaoProduto.php" method="POST">
            <div class="form-group">
                <label>Nome*</label>
                <input type="text" class="form-control" name="nome"  maxlength="50" required value="<?php echo $name; ?>">
                <br>
                <div class="form-group form-check"><?php foreach ($resultado as $linha) { ?>
                        <input type="checkbox" <?php if ($linha['checkbox'] != '') {
                                                            echo "checked";
                                                        } ?> name="ckTag[]" value="<?php echo $linha['id']; ?>" />
                    <?php echo $linha['name'];
                                                    } ?>
                </div>
            </div>

...
if (isset($_POST['submit']) && !empty($_POST['nome'])) {
    if (!empty(($_POST['ckTag']))) {
        $u->conectar("teste-promobit", "localhost", "root", "");
        if ($u->getMsg() == '') {
            $tags = $_POST['ckTag'];
            $nome = addslashes($_POST['nome']);
            if ($u->createProduto($nome, $tags)) {
        ...
        $u->msg("É necessário selecionar pelo menos uma tag para cadastrar o produto.", 'danger');
    }
}
?>
<!DOCTYPE html>
    ...
    <main class="container">
        ...
        <form action="cadastroProduto.php" method="POST">
            <div class="form-group">
                <label>Nome*</label>
                <input type="text" class="form-control" maxlength="50" name="nome" required placeholder="Digite um nome para o produto">
                <br>
                <div class="form-group form-check"><?php foreach ($resultado as $linha) { ?>
                        <input type="checkbox" name="ckTag[]" value="<?php echo $linha['id']; ?>" />
                    <?php echo $linha['name'];
                                                    } ?>
                </div>
            </div>
            <?php echo $u->getMsg() ?>
            <button type="submit" name="submit" class="btn btn-primary">Cadastrar <i class="bi bi-heart-fill"></i></button>
            <a href='listagemProduto.php' class='btn btn-success'>Produtos Cadastrados</a>
        </form>
    </main>
    <br><br>
